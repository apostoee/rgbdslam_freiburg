        ...
        for( int x = 0; x < w-step; x+=step){//for it and its neighbours right and/or down
            ...
            if(!strip_on){ //Generate vertices for new triangle
                ...
                if(!strip_on) { //can't use the point on the upper left, should I still init a triangle?
                  ...
                  if(!validXYZ(*lr)) {
                    ...
                    depth = pcl::squaredEuclideanDistance(*ur,origin);
                    if (pcl::squaredEuclideanDistance(*ur,*ll)/depth <= mesh_thresh  and 
                        pcl::squaredEuclideanDistance(*lr,*ll)/depth <= mesh_thresh  and
                        pcl::squaredEuclideanDistance(*ur,*lr)/depth <= mesh_thresh){
                      glBegin(GL_TRIANGLE_STRIP);
                      strip_on = true;
            ...
            {//neighbours to the left and left down are already set
              ...
              if(validXYZ(*ul)){ //Neighbours to the left are prepared
                depth = pcl::squaredEuclideanDistance(*ul,origin);
                if (pcl::squaredEuclideanDistance(*ul,*(ul-step))/depth > mesh_thresh){
                  glEnd();
                  strip_on = false;
                  continue;
                }
              ...
              if(validXYZ(*ll)){ 
                depth = pcl::squaredEuclideanDistance(*ll,origin);
                if (pcl::squaredEuclideanDistance(*ul,*ll)/depth > mesh_thresh or
                    pcl::squaredEuclideanDistance(*ul,*(ul-step))/depth > mesh_thresh or
                    pcl::squaredEuclideanDistance(*ll,*(ll-step))/depth > mesh_thresh){
                  glEnd();
                  strip_on = false;
                  continue;
                }
...
void GLViewer::pointCloud2GLList(pointcloud_type const * pc){
    ...
    for(unsigned int x = 0; x < w-1; x++){
        for(unsigned int y = 0; y < h-1; y++){
            ...
            if(validXYZ(*pk)
               and pcl::squaredEuclideanDistance(*pi,*pk)/depth <= mesh_thresh  
               and pcl::squaredEuclideanDistance(*pk,*pl)/depth <= mesh_thresh){
